...
import string
from ML import extract_images
import os
from ML import record_audio
#import selecting
# obtain audio from the 


def func(base_dir="ML/"):
        r = sr.Recognizer()
...
'voice', 'wednesday', 'weight','please wait for sometime','what is your mobile number','what are you doing','are you busy']
        ...
        record_audio.record()
        audio_file="media/input/test.wav"
                ...
                while True:
                        ...
                        try:
                                ...
                                if(a.lower()=='goodbye' or a.lower()=='good bye' or a.lower()=='bye'):
                                    ...
                                    lbl = ImageLabel(root)
                                    lbl.pack()
                                    lbl.load(r'{}ISL_Gifs/{}.gif'.format(base_dir,a.lower()))
                                    root.mainloop()
                                else:
                                    for i in range(len(a)):
                                                    if(a[i] in arr):
                                            
                                                            ImageAddress = base_dir+'letters/'+a[i]+'.jpg'
                                                            ...
                                                            continue
                                    extract_images.extract(a)
                                    ext_imgs=list(os.listdir(base_dir+a))
                                    print(ext_imgs)
                        
                                    for i in ext_imgs:
                                        print(i)
                                        ImageAddress = "{}{}/{}".format(base_dir,str(a),str(i))
                                        print(ImageAddress)
                                        ImageItself = Image.open(ImageAddress)
                                        ImageNumpyFormat = np.asarray(ImageItself)
                                        plt.imshow(ImageNumpyFormat)
...
def start_listening(base_dir="ML/"):
    print("Tkinter gui started")
    while 1:
        image   = base_dir+"signlang.png"
        msg="HEARING IMPAIRMENT ASSISTANT"
        choices = ["Live Voice","All Done!"] 
        reply   = buttonbox(msg,image=image,choices=choices)
        if reply ==choices[0]:
